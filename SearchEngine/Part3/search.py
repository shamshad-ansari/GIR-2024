    ...
    for row in metadata:
        if row:
            for keywords in row[-1]:
                keyword_hash_map[keywords] = keyword_hash_map.get(keywords, []) + [row[0]]
...
def title_to_info(metadata):
    ...
    for row in metadata:
        ...
        if row:
            ...
            details['timestamp'] = row[2]
            details['length'] = row[3]
            title_hash_map[row[0]] = details

    return title_hash_map
...
def search(keyword, keyword_to_titles):
    ...
    if keyword and (keyword in keyword_to_titles):
...
def article_length(max_length, article_titles, title_to_info):
    ...
    for title in article_titles:
        ...
        if title_to_info[title]['length'] <= max_length:
            list_of_article_titles.append(title)
...
def key_by_author(article_titles, title_to_info):
    ...
    for title in article_titles:
        ...
        author_hash_map[author] = author_hash_map.get(author,[]) + [title]
...
def filter_out(keyword, article_titles, keyword_to_titles):
    filtered_titles = []
    titles_to_exclude = set(keyword_to_titles.get(keyword, []))
    for title in article_titles:
        if title not in titles_to_exclude:
            filtered_titles.append(title)
    return filtered_titles
...
def articles_from_year(year, article_titles, title_to_info):
    ...
    for title in article_titles:
        ...
        article_year = datetime.datetime.fromtimestamp(timestamp).year
